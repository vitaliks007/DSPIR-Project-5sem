...
foreach ($fixtures->getObjects() as $fixture) {
	$temperature = floor($fixture->temperature);
	$index = array_search($temperature, $uniqueCount);

	if ($index !== false) {
		$points[$index]++;
	} else {
		array_push($points, 1);
		array_push($labels, $temperature);
		array_push($uniqueCount, $temperature);
	}
}
...
$image->drawText(200, 13, "Temperature", ["R" => 0, "G" => 0, "B" => 0, "Align" => TEXT_ALIGN_TOPMIDDLE]);
